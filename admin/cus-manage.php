...
if (isset($_POST['submit'])) {
	...
	$add_sql = "INSERT INTO `khachhang` (hodem, ten, gioitinh, ngaysinh, dienthoai, email, diachi, matkhau) VALUES ('$hodem', '$ten', $gioitinh, '$ngaysinh', '$dienthoai', '$email', '$diachi', '$matkhau')";
...
if (isset($search) && isset($searchValue) && !empty($searchValue)) {
	$sql = "SELECT * FROM khachhang WHERE hodem LIKE '%$searchValue%' OR ten LIKE '%$searchValue%' OR dienthoai LIKE '%$searchValue%' OR diachi LIKE '%$searchValue%'";
	...
	<div class="content">
		...
		<div class="container-fluid">
			...
			<div class="row">
						...
						<tbody>
							...
							?>
								<tr>
									...
									<td>
										<a href="cus-edit.php?id=<?php echo $row['makh']; ?>" class="button btn-primary" name="edit" id="edit"><i class="fa-solid fa-pen-to-square"></i></a>
										<a onclick="return confirm('Bạn có chắc chắn muốn xoá không');" href="cus-delete.php?makh=<?php echo $row['makh']; ?>" class="button btn-danger" name="delete" type="submit" id="delete"><i class="fa-solid fa-trash"></i></a>
									</td>
								</tr>
							<?php }  ?>
						</tbody>
				...
				<?php
				echo "<p align='center'>";
				$re = mysqli_query($con, 'SELECT COUNT(*) as count FROM khachhang');
				...
				if ($search != "")
					echo "Có " . mysqli_num_rows($s_res) . " sản phẩm được tìm thấy";
				else
					echo "";
